...
export class Chem0Backend extends EventEmitter {
  ...
  async streamAgentMessage(input: {
    ...
    try {
      let text = "";
      const instructions =
        "You are controlling a local LeRobot experiment through chem-0. Use tools when hardware state, camera state, arm motion, or human voice interaction is required. Use speak_to_human to talk out loud. Treat listen_to_human transcripts as human messages. Keep motions conservative and prefer known pose-table references.";
